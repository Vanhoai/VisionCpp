...
public:
    explicit MultivariateNormal(const double mean = 0.0, const double standardDeviation = 1.0)
     ...
     * formula: x = mean + L * z
     */
    Eigen::MatrixXd random(int N, const Eigen::VectorXd &mean, const Eigen::MatrixXd &covariance);
