...
public:
    ...
    static Tensor<U> cast(const Tensor& other) {
        ...
        for (size_t i = 0; i < size; ++i) result.data()[i] = static_cast<U>(other.data()[i]);
     ...
     */
    T& operator()(const std::vector<size_t>& indices) { return data_[calculateIndex(indices)]; }
    ...
    T& operator()(const std::size_t row, const std::size_t col) {
        if (shape_.size() != 2) throw std::invalid_argument("Tensor is not 2-dimensional");
    ...
    Tensor operator-(const Tensor& other) const {
        if (shape_ != other.shape_) throw std::invalid_argument("Shape mismatch in operator-");
    ...
    static Tensor dot(const Tensor& A, const Tensor& B) {
        ...
        if (shapeA.size() == 2 && shapeB.size() == 1) {
            if (shapeA[1] != shapeB[0])
                throw std::invalid_argument("Shape mismatch for dot product of 2D and 1D tensors");
        ...
        if (shapeA.size() == 1 && shapeB.size() == 2) {
            if (shapeA[0] != shapeB[0])
                throw std::invalid_argument("Shape mismatch for dot product of 1D and 2D tensors");
    ...
    [[nodiscard]] size_t height() const {
        if (shape_.size() < 2) throw std::runtime_error("Tensor is not 2D or higher");
    ...
    [[nodiscard]] size_t channels() const {
        if (shape_.size() < 3) throw std::runtime_error("Tensor is not 3D or higher");
