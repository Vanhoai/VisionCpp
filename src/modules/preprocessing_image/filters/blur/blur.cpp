        ...
        for (int j = 0; j < inputImage.cols; ++j) {
            ...
            for (int ki = -kernel.rows / 2; ki <= kernel.rows / 2; ++ki) {
                for (int kj = -kernel.cols / 2; kj <= kernel.cols / 2; ++kj) {
                    int ni = i + ki;
                    int nj = j + kj;
                    if (ni >= 0 && ni < inputImage.rows && nj >= 0 && nj < inputImage.cols) {
                        pixelValue += inputImage.at<cv::Vec3b>(ni, nj) *
                                      kernel.at<double>(ki + kernel.rows / 2, kj + kernel.cols / 2);
                    }
                }
            }
...
void test() {
    ...
    std::string path = "/Users/aurorastudyvn/Workspace/ML/CppDeepLearning/image.jpg";
