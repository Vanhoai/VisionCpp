...
Mat SIFTDetector::createGaussianKernel(double sigma, int size) {
    if (size == 0) size = 2 * ceil(3 * sigma) + 1;
    ...
    for (int ds = -1; ds <= 1; ds++) {
        for (int dy = -1; dy <= 1; dy++) {
            for (int dx = -1; dx <= 1; dx++) {
                ...
                const float neighborValue = dogSpace[octave][scale + ds].at<float>(y + dy, x + dx);
                if (centerValue <= neighborValue) isMax = false;
                if (centerValue >= neighborValue) isMin = false;
...
bool SIFTDetector::isValidKeypoint(const vector<vector<Mat>> &dogSpace, int octave, int scale,
    ...
    if (abs(value) < contrastThreshold) return false;
