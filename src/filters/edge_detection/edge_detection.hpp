...
private:
    ...
    int kernelX[3][3] = {
        {-1, 0, 1}, {-2, 0, 2}, {-1, 0, 1}};   // Sobel kernel for X direction
    int kernelY[3][3] = {
        {-1, -2, -1}, {0, 0, 0}, {1, 2, 1}};   // Sobel kernel for Y direction

    void calculateGradient(const Mat &grayImage, Mat &magnitude,
                           Mat &direction);
...
public:
    EdgeDetection(const Mat &inputImage) : image(inputImage) {}
     ...
     */
    void applySobel(Mat &magnitude, Mat &direction, bool dx = true,
                    bool dy = true);
     ...
     * directions
     * - Calculates gradient magnitude (follow the formula of sobel: sqrt(Gx^2 +
     * Gy^2))
     * - Calculates gradient direction (follow the formula of sobel: atan2(Gy,
     * Gx)) and normalizes it to 0-180 degrees
     *
     ...
     * - First pass: Marks pixels above high threshold as strong edges (255),
     * pixels between thresholds as weak edges (128)
     * - Second pass: Uses DFS to connect weak edges that are adjacent to strong
     * edges
