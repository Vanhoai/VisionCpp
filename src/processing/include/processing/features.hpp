...
private:
    ...
    static std::vector<float> computeOrientations(const Keypoint &kp, const Layers &scaleSpace);
    ...
    static void assignOrientations(std::vector<Keypoint> &keypoints, const Layers &scaleSpace);
    static void computeDescriptors(std::vector<Keypoint> &keypoints, const Layers &scaleSpace);
...
public:
    ...
    static bool isFASTCorner(const core::TensorF32 &src, int x, int y, float threshold, int n);
    static float computeCentroidAngle(const core::Tensor<core::float32> &src, int x, int y,
                                      int radius);
    static std::vector<std::pair<std::pair<int, int>, std::pair<int, int>>> generateBRIEFPattern();
    ...
    static void computeOrientations(std::vector<Keypoint> &keypoints, const Layers &pyramid);
