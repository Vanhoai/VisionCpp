...
static std::vector<std::vector<float>> createGaussianKernel(const int ksize, const double sigma) {
...
void Filters::gaussianBlur(const core::Tensor<core::float32>& src, core::Tensor<core::float32>& dst,
                           const int ksize) {
    if (src.empty()) throw std::invalid_argument("Image tensor should not be empty");
...
void Filters::medianBlur(const core::Tensor<core::float32>& src, core::Tensor<core::float32>& dst,
                         const int ksize) {
    const int height = src.shape()[0];
    const int width = src.shape()[1];
    const int channels = src.shape()[2];
...
void Filters::unsharpMask(const core::Tensor<core::float32>& src, core::Tensor<core::float32>& dst,
...
void calculateGradient(const core::Tensor<core::float32>& src,
    ...
    if (src.dimensions() != 2)
        throw std::invalid_argument("Please convert image to grayscale before using this function");
...
void connectWeakEdges(core::Tensor<core::float32>& dst, core::Tensor<core::int8>& visited,
    ...
    if (u < 0 || u >= height || v < 0 || v >= width || visited.at(u, v) == 1) return;
...
void hysteresisThresholding(const core::Tensor<core::float32>& suppressed,
    ...
    for (size_t y = 0; y < height; y++) {
        for (size_t x = 0; x < width; x++) {
            if (dst.at(y, x) == 128.0f) dst.at(y, x) = 0.0f;
        }
    }
}
