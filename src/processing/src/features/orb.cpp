...
namespace processing {
    ...
    bool ORB::isFASTCorner(const core::TensorF32 &src, int x, int y, float threshold, int n) {
        ...
        const int circle[16][2] = {{0, -3}, {1, -3},  {2, -2},  {3, -1}, {3, 0},  {3, 1},
        ...
        if (brightCount >= n) {
            ...
            if (maxBrightSeq >= n || wraparoundBright >= n)
                return true;
        }
        ...
        if (darkCount >= n) {
            ...
            if (maxDarkSeq >= n || wraparoundDark >= n)
                return true;
        }
    ...
    float ORB::computeCentroidAngle(const core::Tensor<core::float32> &src, int x, int y,
                                    int radius) {
    ...
    std::vector<uint8_t> ORB::computeBRIEFDescriptor(const core::TensorF32 &src,
                                                     const Keypoint &kp) {
